...
class CustomerCartController extends Controller
{
    ...
    public function __construct() {
        $this->customer_id= Auth::guard('customers')->id();
        // dd($this->customer_id);
    }
    /**
    ...
    {
        // dd($this->customer_id);
        $carts = Cart::where('customer_id', $this->customer_id)->first();
    ...
    public function store(StoreCartRequest $req)
    {
        ...
        if (DB::table('cart_product')->where('product_id', '=', $req['product_id'])->where('customer_id', '=', $this->customer_id)->exists()) {
        ...
        $total_price_on_cart = $this->countTotalPrice($this->customer_id);
    ...
    public function update(UpdateCartRequest $req)
    {
        ...
        $this->countTotalPrice($this->customer_id);
    ...
    public function destroy(Request $request)
    {
        if($request['id']){
            ...
            if ($total_price_on_cart == 0) {
                ...
                DB::table('carts')->where('customer_id', '=', $this->customer_id)->delete();

                return $this->success('No Products In The Cart');
            }

            DB::table('carts')->where('customer_id', '=', $this->customer_id)->update([
        ...
        DB::table('cart_product')->where('cart_product.customer_id', $this->customer_id)->delete();
     ...
     */
    public static function countTotalPrice($customer_id)
    {
        //get all products belong to specific user then sum all prices of all choosed products
        $totalprice = DB::table('cart_product')->where('customer_id', $customer_id)
            ->sum('total_price');
        if (Cart::where('customer_id', $customer_id)->exists()) {
            DB::table('carts')->where('customer_id', $customer_id)->update(['total_price' => $totalprice, 'updated_at' => now()]);
        } else {
            Cart::insert([
